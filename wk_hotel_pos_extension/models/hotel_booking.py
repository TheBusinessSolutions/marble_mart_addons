...
class HotelBooking(models.Model):
    ...
    def fetch_booked_room_data_for_pos(self):
            ...
            return [{'booking_id':rec.id,'booking_customer':rec.partner_id.name,'customer_id':rec.partner_id.id,'booking_name':rec.sequence_id,'product_data':rec.booking_line_ids.mapped('product_id').read(['name','id'])} for rec in booking_ids]
            # booking_ids = booking.search([('status_bar','not in', ['initial','checkout','cancel'])])
            # return booking_ids.mapped('booking_line_ids').read(['product_id','booking_id'])
